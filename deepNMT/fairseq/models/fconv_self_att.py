...
class FConvModelSelfAtt(FairseqModel):
    ...
    def add_args(parser):
        ...
        parser.add_argument('--not-mask-encoder-future-timesteps', action='store_true', default=False,
                            help='not set that the encoder needs to mask future steps')
...
class FConvEncoder(FairseqEncoder):
    ...
    def __init__(
        ...
        attention_nheads=1, left_pad=True, mask_future_timesteps=True
    ):
        ...
        for i, (out_channels, kernel_size) in enumerate(convolutions):
            ...
            self.attention.append(
                SelfAttention(out_channels, embed_dim, attention_nheads, mask_future_timesteps=mask_future_timesteps) if attention[i] else None
            )
...
class SelfAttention(nn.Module):
    ...
    def __init__(self, out_channels, embed_dim, num_heads, project_input=False, gated=False, downsample=False,mask_future_timesteps=True):
        super().__init__()
    ...
    def forward(self, x):
        ...
        x, _ = self.attention(query, key, value, mask_future_timesteps=self.mask_future_timesteps, use_scalar_bias=True)
...
@register_model_architecture('fconv_self_att', 'fconv_self_wmt_ende')
def fconv_self_wmt_ende(args):
    ...
    args.multihead_attention_nheads = getattr(args, 'multihead_attention_nheads', 8)
    args.multihead_self_attention_nheads = getattr(args, 'multihead_self_attention_nheads', 8)
    base_architecture(args)
    

@register_model_architecture('fconv_self_att', 'fconv_self_small_wmt_ende')
def fconv_self_small_wmt_ende(args):
    convs = '[(512, 3)] * 6'  # first 9 layers have 512 units
    convs += ' + [(1024, 3)] * 4'  # next 4 layers have 1024 units
    convs += ' + [(2048, 1)] * 2'  # final 2 layers use 1x1 convolutions
    args.encoder_embed_dim = getattr(args, 'encoder_embed_dim', 768)
    args.encoder_layers = getattr(args, 'encoder_layers', convs)
