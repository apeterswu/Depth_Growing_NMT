...
def get_parser():
    ...
    parser.add_argument('--monopref', metavar='FP', default=None, help='comma separated, mono language prefixes')
    parser.add_argument('--monopref-out', metavar='FP', default=None, help='comma separated, mono language prefixes')
...
def main(args):
    ...
    def make_mono_binary_dataset(input_file, output_prefix, lang):
        dict = dictionary.Dictionary.load(dict_path(lang))
        print('| [{}] Dictionary: {} types'.format(lang, len(dict) - 1))
    ...
    def make_mono_dataset(input_path, output_prefix, lang):
        if args.output_format == 'binary':
            make_mono_binary_dataset(input_path, output_prefix, lang)
        elif args.output_format == 'raw':
            # Copy original text file to destination folder
            output_text_file = dest_path(output_prefix, lang)
            shutil.copyfile(input_path, output_text_file)

    def make_all(lang):
        if args.trainpref:
            make_dataset(args.trainpref, 'train', lang)
        if args.monopref:
            make_mono_dataset(args.monopref, args.monopref_out, lang)
        if args.validpref:
